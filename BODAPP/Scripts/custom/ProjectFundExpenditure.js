...
function cellAmntChngeForSMMEBudget(input) {
    ...
    if (totalSMMEAmount > activityAmount) {
        Swal.fire({
            title: 'Amount Should Be Less Than ' + activityAmount,
            icon: 'error',
            customClass: { confirmButton: 'btn btn-primary waves-effect waves-light' },
            buttonsStyling: false
        });
...
function cellAmntChngeForExpenditureBudget(input) {
    ...
    if (totalSMMEAmount > projAmnt) {
        Swal.fire({
            title: 'Amount Should Be Less Than ' + projAmnt,
            icon: 'error',
            customClass: { confirmButton: 'btn btn-primary waves-effect waves-light' },
            buttonsStyling: false
        });
...
function cellAmntFortxtExpensesAmntBudget(t) {
    ...
    if (isNaN(txtExpensesAmnt) || txtExpensesAmnt < 0) {
        Swal.fire({
            title: 'Please enter a valid expenditure amount.',
            icon: "error",
            customClass: { confirmButton: "btn btn-primary waves-effect waves-light" },
            buttonsStyling: false
        });
    ...
    if (txtExpensesAmnt > txtTotalSMMEBudget) {
        Swal.fire({
            title: 'Expenditure amount should be less than' + txtTotalSMMEBudget +'',
            icon: "error",
            customClass: { confirmButton: "btn btn-primary waves-effect waves-light" },
            buttonsStyling: false
        });
...
function SaveBudgetDetailsForSMME() {
    ...
    $.each($('#tblSMME tbody tr'), function (index, value) {
        ...
        ExpenditureBudgetArr.push({
            ...
            EWB_Budget: parseFloat($(this).find("td:eq(1) input[type='text']").val()),
            EWB_ExpenditureBudget: parseFloat($(this).find("td:eq(2) input[type='text']").val()),
            EWB_BudgetDistId: parseInt($(this).find("td:eq(3) input[type='hidden']").val()),
            EWB_ActivityId: parseInt($(this).find("td:eq(4) input[type='hidden']").val()),
            EWB_TaskId: parseInt($(this).find("td:eq(5) input[type='hidden']").val())
        });
    });
            ...
            try {
                ...
                $('#ddlSMME').on('change', function () {
                   // $('#tblSMME tbody').html('');
                    ...
                    retriveSMMEBudget(Id, taskId, smmeId);
                });
