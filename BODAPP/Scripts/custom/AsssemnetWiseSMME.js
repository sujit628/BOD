        ...
        success: function (data, status) {
            ...
            i.length && (i = i).wrap('<div class="position-relative"></div>').select2({ placeholder: "Select Country", dropdownParent: i.parent() }),
                n.length &&
                    (e = n.DataTable({
                        data:data,
                        columns: [{ data: "" }, { data: "SMME_CompanyName" }, { data: "SMME_RegNumber" }, { data: "SMME_PrimaryContactNo" }, { data: "View Answer" }],
                        columnDefs: [
                    {
                        className: "control",
                        searchable: !1,
                      ...
                      
                    {
                        ...
                        render: function (data,type,row) {
                            return '<div class="d-flex align-items-sm-center"><a class="btn btn-sm btn-icon btnEdit '+row.ViewAns+'" href="/Assessment/ViewSMMEWiseAssessment?BAId='+ row.AAS_BA_Id+'&Id='+ row.SMME_Id+'"  ><i class="ti ti-eye mx-2 ti-sm"></i></a></div>';
